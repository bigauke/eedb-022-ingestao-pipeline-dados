...
def execute_star_schema_and_final(conn):
    ...
    with conn.cursor() as cur:
        cur.execute("""
            ...
            SELECT 
                t.ano, t.trimestre, t.rotulo_trimestre, 
                COALESCE(i.cnpj_if, 'N/A') AS cnpj_if, 
                i.nome_instituicao, 
                COALESCE(i.segmento, 'Outros') AS segmento,
                COALESCE(i.categoria, 'N/A') AS categoria, 
                COALESCE(i.tipo_instituicao, 'N/A') AS tipo_instituicao, 
                COALESCE(f.indice_reclamacao, 0) AS indice_reclamacao, 
                COALESCE(f.qtd_reclamacoes_procedentes, 0) AS qtd_reclamacoes_procedentes,
                COALESCE(f.qtd_reclamacoes_outras, 0) AS qtd_reclamacoes_outras, 
                COALESCE(f.qtd_reclamacoes_nao_reguladas, 0) AS qtd_reclamacoes_nao_reguladas, 
                COALESCE(f.qtd_total_reclamacoes, 0) AS qtd_total_reclamacoes,
                COALESCE(f.qtd_total_clientes_ccs_scr, 0) AS qtd_total_clientes_ccs_scr, 
                COALESCE(f.qtd_clientes_ccs, 0) AS qtd_clientes_ccs, 
                COALESCE(f.qtd_clientes_scr, 0) AS qtd_clientes_scr, 
                c.employer_name,
